...
const Review = () => {
  return (
    <div className=' h-[100%] w-full flex px-15 py-2 bg-gradient-to-b items-center'>
      {reviews.map((review)=>{
        return (
          <li className='flex -mr-5 last::mr-0' key={review.id}>
            <img className='w-14 h-14 rounded-full border-2 border-[#ffffff] ml-1 ' src={review.src} alt={review.alt} />
          </li>
        )
      })}
